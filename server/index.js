...
const prizes = [
  ...
  '10 convidados + 5 crianças de até 10 anos',
  '25 crianças de 0 a 10 anos'
];
...
function hasRepeatedOrSequentialPattern(text) {
  ...
  const suspicious = ['teste', 'asdf', 'qwerty', 'abc', 'abcd', 'nome', 'fulano', 'cliente'];
...
function validateName(name) {
  ...
  if (value.length < 7) return 'Digite nome e sobrenome.';
  if (hasRepeatedOrSequentialPattern(value)) return 'Digite um nome válido.';
  const parts = value.split(' ');
  if (parts.length < 2) return 'Digite nome e sobrenome.';
  const invalidPart = parts.some((part) => part.length < 2 || /[^a-záàâãéèêíìîóòôõúùûç']/i.test(part));
  if (invalidPart) return 'Digite um nome válido.';
  const allSame = parts.every((part) => part.toLowerCase() === parts[0].toLowerCase());
  if (allSame) return 'Digite um nome válido.';
  return null;
}
...
function validateWhatsApp(whatsapp) {
  ...
  if (ddd < 11 || ddd > 99) return 'Digite um WhatsApp válido com DDD.';
  if (digits[2] !== '9') return 'Digite um WhatsApp celular válido.';
  if (/^(\d)\1+$/.test(digits)) return 'Digite um WhatsApp válido.';
  if (/(01234|12345|23456|34567|45678|56789|67890|98765)/.test(digits)) return 'Digite um WhatsApp válido.';
  return null;
}
